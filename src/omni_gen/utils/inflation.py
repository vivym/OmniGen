...
def inflate_params_from_2d_vae(
    ...
    need_key_alignment = "encoder.mid.attn_1.q.weight" in vae_2d_params

    for key_3d in vae_3d_params.keys():
        if key_3d.startswith("loss") or key_3d.endswith(".scale") or ".upsampler.conv2." in key_3d:
            continue

        key_2d = key_3d

        if need_key_alignment:
